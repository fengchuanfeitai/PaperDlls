        ...
        try
        {
            for (int num = 0; num < ParaList.size(); num++)
            {
                ...
                switch (ParaList.get(num)._Formula)
                {
                    ...
                    case "DtVFunc":
                    {
                        ParaList.set(num,Function.DtVFunc(ParaList.get(num), DStoPP(ParaList.get(num)._DataSourse)));
                        break;
                    }
                    case "SlopeFunc":
                    {
                        ParaList.set(num,Function.SlopeFunc(ParaList.get(num), DStoPP(ParaList.get(num)._DataSourse)));
                        break;
                    }

                    case "PlonkValueFunc":
                    {
                        ParaList.set(num,Function.PlonkValueFunc(ParaList.get(num), DStoPP(ParaList.get(num)._DataSourse)));
                        break;
                    }

                    case "PlonkErrorFunc":
                    {
                        ParaList.set(num,Function.PlonkErrorFunc(ParaList.get(num), DStoPP(ParaList.get(num)._DataSourse)));
                        break;
                    }
                    default:
                        break;
