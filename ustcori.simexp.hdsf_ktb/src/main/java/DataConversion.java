...
public class DataConversion {
    ...
    public static int High(String str) {
        ...
        try {
            if ("+".equals(str.substring(0, 1)) || "-".equals(str.substring(0, 1))) {
                str = str.substring(1, str.length());//去除符号位
            }
            String[] HL = str.split(".");
            ...
            if (HL.length > 1) {
                if (high > 0) {
                    ...
                    while (pos < HL[1].length()) {
                        String ss = HL[1].substring(pos, pos + 1);
                        if (!"0".equals(ss)) {
                            pos = -(pos + 1);
                            break;
                        } else {
    ...
    public static String SingleDoubleOfDecimal(String result, int num) {
        ...
        if (DataValidity.IsNumeric(result).isValid) {
            ...
            //取舍之前要保留到的末位值
            String lastStr = temp.substring(PointPos + num, PointPos + num + 1);
            //取舍之前要保留到的末位的后一位值
            String nextStr = temp.substring(PointPos + num + 1, PointPos + num + 2);
            int nextPos = PointPos + num + 1;
            if (lastStr.equals(".")) {
                //加1去除小数点
                lastStr = temp.substring(PointPos + num - 1, PointPos + num);
            }
            //末尾为偶数
            if (Integer.parseInt(lastStr) % 2 == 0) {
                if (Integer.parseInt(nextStr) != 5) {
                    ...
                    String pl = temp.substring(nextPos + 1, temp.length());
    ...
    /// <summary>
    /// 保留数值有效位数，首位或等于3保留一位有效值，首位小于3保留2位有效值(进位规则余位进一)
    /// </summary>
    /// <param name="result">待截取的数值</param>
    /// <returns>截取后的结果</returns>
    public static String PrimacyEffective(double result) {
        String temp = "";
        try {
            if (result <= 0.0000000000000009) {
                temp = "0";
            } else if (result <= 29) {
                ...
                int c = 0;
                if (num < 0) {
                    c = Integer.valueOf(b.substring(-num + 1, -num + 2));//获取首位值
                } else {
                    c = Integer.valueOf(b.substring(0, 1));//获取首位值
                }
                if (c == 1 || c == 2) {
                    temp = SingleDoubleOfEffective(String.valueOf(result), 2);
                    //判断四舍五入后所得值是否符合首位大于或等于3保留1位，首位小于3保留2位
                    String b1 = String.valueOf(temp);
                    int num1 = High(b1);
                    int c1 = 0;
                    if (num1 < 0)//若为小数，获取有效数值
                    {
                        c1 = Integer.valueOf(b1.substring(-num1 + 1, -num1 + 2));//获取首位值
                    } else {
                        c1 = Integer.valueOf(b1.substring(0, 1));//获取首位值
                    }
                    if (c1 >= 3) {
                        temp = SingleDoubleOfEffective(String.valueOf(b1), 1);//获取首位值
                    }

                } else if (c >= 3) {
                    temp = SingleDoubleOfEffective(String.valueOf(result), 1);
                    //判断四舍五入后所得值是否符合首位大于或等于3保留1位，首位小于3保留2位
                    String b1 = String.valueOf(temp);
                    int num1 = High(b1);
                    int c1 = 0;
                    ...
                    if (c1 == 1 || c == 2) {
                        temp = SingleDoubleOfEffective(String.valueOf(b1), 2);
                    }
    ...
    /// <summary>
    /// 获取表达式结果，进位规则为奇进偶退,进位方式为靠近末位高的一方
    /// </summary>
    /// <param name="Xp">准确值</param>
    /// <param name="Ux">不确定度</param>
    /// <returns></returns>
    public static String GetExpressionToEnd(String Xp, String Ux) {
        String temp = "";
        ...
        try {
            Sx = Xp;
            Su = Ux;
            int nx = GetDecimalPointDigits(Sx);
            int nu = GetDecimalPointDigits(Su);
            int nmin = nx;
            if (nx > nu) {
                nmin = nu;
            }
            temp = SingleDoubleOfDecimal(Sx, nmin) + "±" + SingleDoubleOfDecimal(Su, nmin);
        } catch (Exception exception) {
            temp = "";
        }
    ...
    /**
     * 判断字符串是否或null * * @param string 字符串 * @return 结果
     */
     ...
     * @return
     */
    public static String GetExpressionBKD(String Xp, String Ux, int Unum) {
        String temp = "";
        ...
        try {
            ...
            if (ylow < 0) {
                Sx = CutDataDecimal(Sx, Math.abs(ylow));
                temp = Sx + "±" + Su;
                return temp;
            } else {
                int k = yhigh - Unum + 1;
                temp = CutDataDecimal(String.valueOf(Double.parseDouble(CutDataDecimal(String.valueOf(Double.parseDouble(Sx) / Math.pow(10, k)), 0)) * Math.pow(10, k)), 0) + "±" + Su;
                return temp;
            }
        } catch (Exception ex) {
            temp = "";
